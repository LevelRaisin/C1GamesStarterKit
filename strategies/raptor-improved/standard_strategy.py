...
def prepare_emp_attack(game_state, state, emps_required):
    # piston:
    game_state.attempt_spawn(FILTER, [21,8])
    if not game_state.game_map[21,8]: return False

    bits_needed = emps_required * 3
    bits_next_turn = game_state.project_future_bits(1, 0)
    if bits_next_turn < bits_needed: return False

    gamelib.debug_write(f"Preparing emp attack: {emps_required} EMPs needed for successful raid.")
    hole = get_hole(state)
    game_state.attempt_remove([hole])
    state["preset_attack"] = partial(launch_emp_attack, hole, emps_required)
    state["is_emp_attacking"] = True
    return True
...
def launch_emp_attack(hole, emps_required, game_state, state):
    ...
    state["is_emp_attacking"] = False
    state["preset_attack"] = None
    state["emp_round"] += 1

    if hole not in path:
        gamelib.debug_write(f"[Error]: EMP Launch failed, path obstructed.")
        return False
